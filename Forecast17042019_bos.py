...
def main():
    ...
    # Read config file
    print(f"Config File Complete Path:\n{os.path.join(filepath, configfilename)}")
    ...
    df_test = df[(df['year'] == '2018')]  # Don't add another year
    ...
    total_pred = sum(preds['pred'])

    # Find BOS
    df_2k16 = df[df['year'] == '2016'].copy()
    df_2k17 = df[df['year'] == '2017'].copy()
    sum_2k16 = sum(df[df['year'] == '2016']['Sale'])
    sum_2k17 = sum(df[df['year'] == '2017']['Sale'])
    df_2k16['BOS'] = df_2k16['Sale'].apply(lambda i: i / sum_2k16)
    df_2k17['BOS'] = df_2k17['Sale'].apply(lambda i: i / sum_2k17)
    df_2k16.reset_index(inplace=True, drop=True)
    df_2k17.reset_index(inplace=True, drop=True)

    # Use BOS
    # predictions = pd.DataFrame(df_2k16['BOS'] * total_pred)
    predictions = pd.DataFrame((df_2k17['BOS'] + df_2k16['BOS']) * total_pred / 2)
    predictions = predictions['BOS'].tolist()

    # Construct analytics
    sales_analytics = pd.DataFrame(df_test['Month'])
    sales_analytics['Actual'] = df_test['Sale']
    sales_analytics['Predict'] = predictions
